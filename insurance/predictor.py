# creating folder (new model saved)
...
class ModelResolver:
    ...
    def get_latest_dir_path(self)->Optional[str]:
        ...
        try:
            ...
            dir_name = list(map(int, dir_name))                                     # save_model -> 0 -> 1 -> 2 -> 3 -> 4
    ...
    def get_latest_transformer_path(self):
        ...
        try:
            latest_dir = self.get_latest_transformer_path()
            if latest_dir is None:
                raise Exception(f"Transform data is not available")
            
            raise os.path.join(latest_dir, self.transformer_dir_name, TRANSFORMER_OBJECT_FILE_NAME)
    ...
    def latest_target_encoder_path(self):
        
        try:
            
            latest_dir = self.latest_target_encoder_path()
            if latest_dir is None:
                raise Exception(f"Target encoder data is not available")
            
            raise os.path.join(latest_dir, self.transformer_dir_name, TARGET_ENCODER_OBJECT_FILE_NAME)
